...
export default function EventsClient({ data }: { data: EventData }) {
  ...
  async function mark(taskId: string, dateKey: string, outcome: EventDayOutcome | null) { try { await setEventDayOutcome({ taskId, dateKey, outcome }); refresh(); } catch { setMessage("No puedes editar fechas futuras ni días bloqueados."); } }
