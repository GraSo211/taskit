...
export default function LandingPage() {
  return <main className="paper-grain min-h-screen overflow-hidden bg-black">
    <header className="mx-auto flex max-w-7xl items-center justify-between px-4 py-5 sm:px-10 sm:py-8">
      <BrandMark />
      <nav className="flex items-center gap-4 text-sm sm:gap-8" aria-label="Acceso a tu cuenta">
        ...
        <Link href="/sign-in" className="iris-button inline-flex min-h-11 items-center px-5 text-xs font-semibold uppercase tracking-[.08em]">Iniciar sesión</Link>
      </nav>
    </header>
    <section className="mx-auto grid min-h-[calc(100vh-80px)] max-w-7xl items-center gap-8 px-4 pb-12 pt-8 sm:gap-12 sm:px-10 sm:pb-24 sm:pt-20 lg:min-h-[calc(100vh-108px)] lg:grid-cols-[.95fr_1.05fr] lg:gap-16">
      <div className="relative z-10">
        <p className="text-xs font-semibold uppercase tracking-[.2em] text-[#ffb829] sm:text-sm">Tareas que encuentran su ritmo</p>
        <h1 className="display-type mt-6 max-w-3xl text-[clamp(3rem,12vw,7rem)] leading-[.91] text-white sm:mt-7">Haz espacio para lo que importa<span className="text-[#8052ff]">.</span></h1>
        ...
        <div className="mt-8 flex flex-wrap items-center gap-5 sm:mt-10"><Link href="/sign-in" className="iris-button inline-flex min-h-12 items-center px-6 text-sm font-semibold uppercase tracking-[.08em]">Entrar ahora</Link></div>
      </div>
      <div className="relative min-h-[220px] sm:min-h-[460px]" aria-hidden="true">
