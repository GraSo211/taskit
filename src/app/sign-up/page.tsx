...
export default function SignUpPage() {
  ...
  async function submit(event: FormEvent<HTMLFormElement>) { event.preventDefault(); setBusy(true); setError(""); const data = new FormData(event.currentTarget); try { const result = await signUp.email({ name: String(data.get("name")), email: String(data.get("email")), password: String(data.get("password")), callbackURL: "/dashboard" }); if (result.error) setError(formatAuthError(result.error, "No hemos podido crear tu cuenta. Revisa los datos o la configuración del servidor.")); else router.push("/dashboard"); } catch (error) { setError(formatAuthError(error, "No hemos podido crear tu cuenta. Revisa la conexión y la configuración del servidor.")); } finally { setBusy(false); } }
